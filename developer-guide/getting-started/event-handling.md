...
```javascript
    myEventHandler: function (sender, ev) {
...
* `ev` is an object that contains event data.  For DOM events, this is the
...
`ev.stopPropagation()` will not prevent propagation of events in Enyo;
...
```javascript
    ...
    anythingTap: function (sender, ev) {
        // do stuff
    }
```
...
`ev.originator` properties to help you discern the provenance of the event.
...
```javascript
    ...
    module.exports = kind({
        ...
        transmission: function (sender, ev) {
...
`handleKeyUp()`), `ev.keyCode` is the JavaScript key code representing the
key that was pressed.  In the `onkeypress` handler (`handleKeyPress()`),
`ev.charCode` is the decimal value of the Unicode character generated by
the keypress; you can get the character itself as a string by passing
`ev.charCode` into `String.fromCharCode()`.

For example, let's say I press the `J` key on my keyboard to type a lowercase
"j".  First, an `onkeydown` event is fired, in which `ev.keyCode` has a
...
an `onkeypress` event is fired, in which `ev.charCode` has a value of 
`106`, which is the decimal value of the Unicode character lowercase "j".
Finally, an `onkeyup` event is fired, in which, once again, `ev.keyCode` is
`74`.
...
`ev.keyCode` has a value of `16`, representing the SHIFT key, and there is
no associated `onkeypress` event since there is no character generated.  In the
second `onkeydown` and `onkeyup`, `ev.keyCode` has a value of `74`, which
...
`ev.charCode` is also `74`, representing the decimal value of the Unicode
